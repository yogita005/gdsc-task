...
const Modal = ({ isSettingsOn, setIsSettingsOn, setPomodoro }) => {
  ...
  return (
    <>
      {isSettingsOn && (
        <div
          className={`block modal absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white w-[20rem] md:w-[28rem] rounded-2xl text-cyan-800 px-6 pt-6 pb-12`}
          ref={modalRef}>
          ...
          <div>
            ...
            <form
              ...
              <button
                type="submit"
                className="absolute -bottom-5 bg-salmon-red-300 text-mint-800 font-semibold text-sm rounded-full px-8 py-3 left-1/2 -translate-x-1/2 hover:bg-salmon-red-600 transition-all cursor-pointer">
                Apply
              </button>
            </form>
          </div>
